...
from memstrata.lib.media import MediaInfo, probe_media
from .schemas import BoundaryReason, BoundarySource, SegmentBoundary, SbdResult
...
def detect_shot_boundaries(
    ...
    *,
    ...
    candidates: list[tuple[str, Callable[[], list[SegmentBoundary]]]]
    if method == "auto":
        candidates = [
            (
                "transnetv2", 
...
def _detect_with_transnetv2(
    ...
    *, 
    ...
    try:
        from memstrata.extras.shot_boundary.TransNetV2 import TransNetV2
...
def _detect_with_ffmpeg(video_path: Path, *, threshold: float) -> list[SegmentBoundary]:
