"""Line art (scribble) layout anchor processor.
...
class LineArtProcessor(BaseLayoutProcessor):
    """Processor that renders layout elements as outlines for Qwen-Image-Edit."""
    ...
    def render_anchor(
        ...
        """Render the layout elements onto a PIL image canvas as outlines."""
        w = width or self.default_width
        h = height or self.default_height
        bg = background_color or (255, 255, 255)  # Default to clean white background
        ...
        for elem in elements:
            ...
            self._draw_element(draw, xmin, ymin, xmax, ymax, elem, line_width)
    ...
    def _draw_element(
        ...
        """Internal helper to draw outlines."""
        ...
        if shape == "rectangle":
            ...
            draw.line((xmin, ymin, xmax, ymax), fill=color, width=max(line_width, int(min(w, h) * 0.1)))
        elif shape in {"ellipse_and_rect", "human", "person", "actor", "speaker"}:
            # Draw a beautifully scaled, simplified outline human figure
            ...
            draw.ellipse((head_xmin, head_ymin, head_xmax, head_ymax), outline=color, width=line_width)
            ...
            draw.rectangle((torso_xmin, torso_ymin, torso_xmax, torso_ymax), outline=color, width=line_width)

            # 3. Legs (lines, bottom 40% of height)
            leg_width = max(line_width, int(w * 0.1))
            # Left leg
            draw.line(
