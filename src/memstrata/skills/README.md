...
`upstream skills` layout, and avoids importing `montage` internals (helpers are
...
| `layout_anchor_processing` | adapted from `upstream skills` + local `crop2image` | PIL, `memstrata.mllm` | R3 layout, R4 crop→region, FLUX I2I keyframe |
| `focus_segmentation` | verbatim `upstream skills` | numpy only | clean-crop extraction, decompose focus/QA |
| `embedding_deduplication` | `upstream skills`, import rewired to `memstrata.encoders.base` | none (stdlib) | curate/R9 crop dedup, non-redundant selection |

## upstream skills copy candidates (not yet vendored)
