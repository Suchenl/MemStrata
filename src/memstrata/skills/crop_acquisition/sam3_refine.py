"""SAM3 geometric refinement of proposed crop boxes and points.

Vendored from bench S5 ``sam3_refine``. Rewrites vs. bench: geometry helpers +
``import_sam3_classes`` from the vendored crop_acquisition package, and
``public_models_root`` from vendored ``_common``. torch/transformers stay lazy.
"""
...
from memstrata.skills.crop_acquisition.sam3_common import import_sam3_classes
from memstrata.skills.crop_acquisition._common import public_models_root
...
class Sam3BoxPointRefiner:
    """Refine a proposed box using SAM3 and select a mask with the positive point.

    HF SAM3 currently exposes box prompts.  The positive point is still consumed:
