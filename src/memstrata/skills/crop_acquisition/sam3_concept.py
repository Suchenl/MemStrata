"""SAM3 open-vocab concept segmentation (vendored from bench S5 ``sam3_concept``).

Rewrites vs. bench: ``import_sam3_classes`` from vendored ``sam3_common`` and
``public_models_root`` from vendored ``_common`` (SAM3 default
``PUBLIC_MODELS_ROOT/facebook/sam3``). torch/transformers stay lazy.
"""
