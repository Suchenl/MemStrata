"""Exemplar identity assignment for crop acquisition (vendored from bench S5).

SAM3/GDINO only propose regions; WHO each crop is comes from DINOv3 similarity to
already-accepted exemplar crops. Unmatched candidates stay unassigned.

Only rewrite vs. bench: ``cosine_similarity`` now comes from the vendored
``memstrata.skills.crop_acquisition._common`` (no ``vmem_bench`` import).
"""
...
from memstrata.skills.crop_acquisition._common import cosine_similarity
...
def max_cosine_to(candidate_vec: list[float], reference_vecs: list[list[float]]) -> float:
    """Max cosine similarity of ``candidate_vec`` to any of ``reference_vecs`` (−1 if empty)."""
    best = -1.0
    for ref in reference_vecs:
        if not ref:
            continue
        best = max(best, cosine_similarity(list(candidate_vec), list(ref)))
    return best
