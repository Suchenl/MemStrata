...
Memory Update — the paper **Stratified Update** step (Step 4), i.e. the
**commit-to-bank (persistence)** logic, formalized as a reusable skill.
`memstrata.steps.curate` is now a thin re-export shim.
...
| stage | behavior |
|---|---|
| identity merge | name-anchored: explicit `entity_id` first, else same `name`+`kind`; aliases are recorded (leaving a trace) |
...
| lifecycle | state events deprecate reps with traceable `deprecated_by` (leaves a trace) + `replaced_by` relation chains; reversible, never silently deleted |
