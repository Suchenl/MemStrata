...
本分支 **不是** 生产 `main`。它是论文 **Track A Stage 1**
（91 movies / MemStrata `__B16`）使用的公开冻结代码树。
...
- **Track A Stage 1（本仓职责）**：方法侧入口是 `python3 -m memstrata.production.run`；评测 adapter 在 **VMem-Bench** 仓的 `paper-reproduction`：
...
## 可复制的 Track A Stage-1 命令

将本仓库与 VMem-Bench 的 `paper-reproduction` 分支并列放置，并从
VMem-Bench 目录运行冻结 adapter：
...
命令需要对应的源视频；产物会写入影片 run 目录中的
`visual_selections` 与 manifest。随后按 VMem-Bench 的
[`REPRODUCE.md`](https://github.com/Suchenl/VMem-Bench/blob/paper-reproduction/REPRODUCE.md)
运行 `visual_coverage`。论文设置使用 `--budget 16`，更小的 budget 仅用于冒烟或消融。
